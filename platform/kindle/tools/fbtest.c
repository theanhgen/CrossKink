...
static void px(int x, int y, unsigned char nib) {
  ...
  if (x & 1)
    *b = (unsigned char)((*b & 0xF0) | (nib & 0x0F));
  else
    *b = (unsigned char)((*b & 0x0F) | ((nib & 0x0F) << 4));
}
...
int main(int argc, char** argv) {
  ...
  if (v.bits_per_pixel != 4) {
    printf("not 4bpp - aborting\n");
    close(fd);
    return 1;
  }
  ...
  if (ioctl(fd, FBIO_EINK_UPDATE_DISPLAY, FX_UPDATE_FULL) < 0) printf("update ioctl failed\n");
