...
static void phase(int n, const char* what, size_t off, size_t len, void (*trig)(void)) {
  printf("phase %d: %s\n", n, what);
  fflush(stdout);
  fill_plane(off, len, 0x0);
  trig();
  sleep(6); /* black */
  fill_plane(off, len, 0xF);
  trig();
  sleep(3); /* white */
}
