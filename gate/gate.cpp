...
int main() {
  // --- environment ---------------------------------------------------
  utsname u{};
  if (uname(&u) == 0) std::printf("[INFO] kernel  %s %s (%s)\n", u.sysname, u.release, u.machine);
  ...
  try {
    ...
    caught = (std::strcmp(e.what(), "gate") == 0);
  }
  ...
  try {
    ...
    check("std::locale(\"C\")", true);
  } catch (const std::exception& e) {
    check("std::locale(\"C\")", false, e.what());
  }
  // std::locale("") is EXPECTED to throw on device -- informational only.
  try {
    std::locale l("");
    std::printf("[INFO] locale(\"\") ok: %s\n", l.name().c_str());
  } catch (const std::exception& e) {
    std::printf("[INFO] locale(\"\") threw (expected): %s\n", e.what());
  }
